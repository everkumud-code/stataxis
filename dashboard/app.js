(() => {
  ...
  const renderComparison = (payload) => {
    ...
    [["last_3_weeks", "3 Weeks", "#hist-3w"], ["last_1_month", "1 Month", "#hist-1m"], ["last_1_year", "1 Year", "#hist-1y"]].forEach(([key, label, selector]) => {
      ...
      const node = $(selector);
      if (node) node.textContent = change.score_delta == null ? "—" : signed(change.score_delta);
      const meta = $(`${selector}-meta`);
      if (meta) meta.textContent = change.sufficient_data ? `${label} · ${change.score_percent_change == null ? "" : signed(change.score_percent_change) + "%"}` : `${label} · insufficient history`;
    });
  };
  ...
  const renderSignals = (items) => {
    ...
    if (!Array.isArray(items) || !items.length) {
      container.innerHTML = `<div><span>No persisted drivers</span><i style="width:4%"></i><b>—</b></div>`;
      return;
    }
    ...
    container.innerHTML = items.slice(0, 5).map((item) => {
      ...
      return `<div><span>${item.name}</span><i style="width:${width}%"></i><b>${signed(value)}</b></div>`;
    }).join("");
  };
  ...
  const renderSeries = (payload) => {
    ...
    if (axis) axis.innerHTML = `<span>${compact(max)}</span><span>${compact((max + min) / 2)}</span><span>${compact(min)}</span>`;
    ...
    setText("#audience-value", compact(values[values.length - 1]));
    setText("#audience-delta", `${compact(values[values.length - 1])} cumulative views`);
    if (note) note.textContent = `${points.length} persisted observation points · cumulative view count`;
  };

  const renderSignalsCard = (payload) => {
    const velocity = Number(payload?.average_view_velocity_per_minute);
    const direction = payload?.momentum_direction;
    const velocityText = Number.isFinite(velocity) ? `${velocity >= 0 ? "+" : ""}${compact(Math.abs(velocity))}/min` : "—";
    setText("#momentum-value", velocityText);
    setText("#momentum-direction", direction === "positive" ? "RISING" : direction === "negative" ? "FALLING" : direction === "flat" ? "FLAT" : "STX");
    setText("#momentum-meta", Number.isFinite(Number(payload?.audience_momentum_per_minute)) ? `Audience momentum ${signed(Number(payload.audience_momentum_per_minute), 2)}/min` : "View velocity from persisted observations");
  };

  const renderCompetition = (payload, selectedChannelId) => {
    const channels = Array.isArray(payload?.channels) ? payload.channels : [];
    const current = channels.find((item) => String(item.channel_id) === String(selectedChannelId));
    const container = $("#competition-rows");
    if (!container) return;
    if (!current || !current.current) {
      container.innerHTML = `<div><span>Channel comparison</span><b>—</b><em>→</em></div><div><span>Rank movement</span><b>—</b><em>→</em></div><div><span>Share movement</span><b>—</b><em>→</em></div>`;
      setText("#competitive-rank", "—");
      setText("#competitive-share", "No current STX comparison");
      setText("#competitive-meta", "Requires scored channels");
      return;
    }
    const rank = Number(current.rank);
    const score = Number(current.current.score);
    const leader = channels[0];
    const leaderScore = Number(leader?.current?.score);
    const gap = Number.isFinite(leaderScore) ? score - leaderScore : null;
    setText("#competitive-rank", Number.isFinite(rank) ? `#${rank}` : "—");
    setText("#competitive-share", gap == null || rank === 1 ? (rank === 1 ? "#1 STX position" : "Relative comparison") : `Gap to leader ${signed(gap)}`);
    setText("#competitive-meta", `${channels.length} scored channel${channels.length === 1 ? "" : "s"} in comparison`);
    const monthChange = current.last_1_month?.change?.score_delta;
    const rows = [
      ["Channel comparison", rank === 1 ? "Leader" : `#${rank}`, "→"],
      ["Rank movement", current.last_1_month?.change?.sufficient_data ? "Tracked" : "—", current.last_1_month?.change?.sufficient_data ? "✓" : "→"],
      ["STX movement", monthChange == null ? "—" : signed(monthChange), monthChange == null ? "→" : monthChange >= 0 ? "↗" : "↘"],
    ];
    container.innerHTML = rows.map(([label, value, arrow]) => `<div><span>${label}</span><b>${value}</b><em>${arrow}</em></div>`).join("");
    setText("#competition-note", payload?.as_of ? `As of ${new Date(payload.as_of).toLocaleString("en-IN")}` : "Current relative position");
  };
  ...
  const renderChannel = async (channel, days = 30) => {
    ...
    const [comparison, series, signals, allComparison] = await Promise.all([
      loadJson(`/api/v1/channels/${channel.id}/comparison`),
      loadJson(`/api/v1/channels/${channel.id}/series?days=${days}`),
      loadJson(`/api/v1/channels/${channel.id}/signals?hours=24`),
      loadJson(`/api/v1/channels/compare?ids=${channel.id}`),
    ]);
    renderComparison(comparison);
    renderSeries(series);
    renderSignalsCard(signals);

    const selectedIds = (window.__stataxisChannels || []).slice(0, 12).map((item) => item.id);
    const competition = selectedIds.length > 1 ? await loadJson(`/api/v1/channels/compare?ids=${selectedIds.join(",")}`) : allComparison;
    renderCompetition(competition, channel.id);
  };
  ...
  const boot = async () => {
    ...
    try {
      ...
      window.__stataxisChannels = channels;
      ...
      await refresh();
      select.addEventListener("change", () => refresh().catch(() => { status.textContent = "API unavailable"; dot.classList.remove("connected"); }));
      filters.forEach((button) => button.addEventListener("click", () => {
        const label = button.textContent.trim();
        activeDays = periodDays[label] || 30;
        filters.forEach((item) => item.classList.toggle("active", item === button));
        refresh().catch(() => { status.textContent = "API unavailable"; dot.classList.remove("connected"); });
      }));
    } catch (error) {
