...
def market_report(
    ...
    *,
    ...
    for rank, item in enumerate(ranked, start=1):
        ...
        item["rank_change"] = (
            item["previous_rank"] - rank if item["previous_rank"] is not None else None
        )
    ...
    peak_total = max(
        (item["peak_concurrent"] for item in ranked if item["peak_concurrent"] is not None),
        default=None,
    )
    return {
...
def channel_media_intelligence(
    ...
    *,
    ...
    aggregate = _aggregate_channel(channel, rows, stream_scope)
    videos = _video_performance(rows, top_videos)
    return {
        "channel": {
            "id": channel.id,
...
def _eligible_channels(session: Session, *, language: str | None, region: str | None, market: str | None) -> list[Channel]:
    ...
    for channel in channels:
        ...
        if language and channel_language.lower() != language.lower():
            continue
        if region and (channel.region or "unknown").lower() != region.lower():
            continue
        if market and _market_name(channel).lower() != market.lower():
            continue
...
def _observation_rows(session: Session, channel_ids: list[int], start_at: datetime, end_at: datetime) -> list[dict[str, Any]]:
    ...
    stmt = (
        ...
        .where(
            Observation.channel_id.in_(channel_ids),
...
def _aggregate_by_channel(session: Session, channels: list[Channel], rows: list[dict[str, Any]], stream_scope: str) -> dict[int, dict[str, Any]]:
    ...
    for row in rows:
        if _scope_matches(row, stream_scope):
...
def _aggregate_channel(channel: Channel, rows: list[dict[str, Any]], stream_scope: str) -> dict[str, Any]:
    ...
    uploads = {
        ...
        if row["published_at"] and _published_in_window(row["published_at"], rows)
    }
...
def _scope_matches(row: dict[str, Any], stream_scope: str) -> bool:
    ...
    if scope == "all":
        return True
    if scope == "live":
        return bool(row["is_live"] or row["classification"] == "LIVE")
    if scope == "replay":
        return row["classification"] == "COMPLETED_LIVE"
    if scope == "video":
        return row["classification"] in {"REGULAR_VIDEO", "SHORT", "PREMIERE", "UNKNOWN"}
...
def _period_start(as_of: datetime, period: str) -> datetime:
    ...
    if period not in PERIOD_DAYS:
        raise ValueError("period must be one of: 1h, 3h, 6h, today, 7d, week, 3w, month, quarter, year")
    if period == "today":
...
def _window_days(rows: list[dict[str, Any]]) -> float:
    if len(rows) < 2:
        return 1.0
    span = max((max(row["observed_at"] for row in rows) - min(row["observed_at"] for row in rows)).total_seconds() / 86400, 1 / 24)
    return span
