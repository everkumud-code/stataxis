...
def collection_health(session: Session, *, as_of: datetime | None = None, stale_after_minutes: int = 360) -> dict[str, Any]:
    ...
    return {"status": run.status, "fresh": fresh, "as_of": resolved_as_of.isoformat(), "latest_run": {"id": run.id, "started_at": started_at.isoformat(), "finished_at": finished_at.isoformat() if finished_at else None, "status": run.status, "channels_attempted": run.channels_attempted, "videos_observed": run.videos_observed, "error_message": run.error_message, "age_minutes": round(age_minutes, 2)}, "stale_after_minutes": stale_after_minutes}
...
def intelligence_readiness(session: Session, *, as_of: datetime | None = None, stale_after_minutes: int = 360, min_snapshot_coverage: float = 1.0) -> dict[str, Any]:
    ...
    integrity_payload = {"ok": integrity.ok, "checked_channels": integrity.checked_channels, "checked_videos": integrity.checked_videos, "checked_observations": integrity.checked_observations, "checked_collection_runs": integrity.checked_collection_runs, "issue_count": len(integrity.issues), "issues": [{"code": issue.code, "message": issue.message} for issue in integrity.issues]}
    run = health["latest_run"]
    if not integrity.ok:
        observed = 0 if run is None else max(0, int(run["videos_observed"]))
        return {"ready": False, "reason": "data_integrity_failed", "collection": health, "integrity": integrity_payload, "intelligence": {"videos_observed": observed, "videos_with_intelligence": 0, "coverage": 0.0, "min_snapshot_coverage": min_snapshot_coverage}}
    if run is None:
    ...
    observed_video_ids = select(Observation.video_id.label("video_id")).where(Observation.observed_at >= run_started_at).where(Observation.observed_at <= resolved_as_of).distinct().subquery("observed_video_ids")
    query = select(func.count(distinct(IntelligenceSnapshotRecord.video_id))).select_from(IntelligenceSnapshotRecord).where(IntelligenceSnapshotRecord.video_id.in_(select(observed_video_ids.c.video_id))).where(IntelligenceSnapshotRecord.generated_at >= run_started_at).where(IntelligenceSnapshotRecord.generated_at <= resolved_as_of)
    ...
    reason = "ready"
    if not health["fresh"]: reason = "collection_not_fresh"
    elif observed == 0: reason = "no_videos_observed"
    elif coverage < min_snapshot_coverage: reason = "intelligence_coverage_below_threshold"
    return {"ready": ready, "reason": reason, "collection": health, "integrity": integrity_payload, "intelligence": {"videos_observed": observed, "videos_with_intelligence": with_intelligence, "coverage": round(coverage, 4), "min_snapshot_coverage": min_snapshot_coverage}}
