...
def test_register_creates_free_account(tmp_path, monkeypatch):
    ...
    payload = register(session, " Test@Example.com ", "correct horse battery")
...
def test_duplicate_registration_rejected(tmp_path, monkeypatch):
    monkeypatch.setenv("STAXIS_AUTH_SECRET", "test-secret")
    session = make_session(tmp_path)
    register(session, "test@example.com", "correct horse battery")
    with pytest.raises(ValueError, match="already registered"):
        register(session, "TEST@example.com", "correct horse battery")
    session.close()


def test_login_issues_token_and_authenticates(tmp_path, monkeypatch):
    monkeypatch.setenv("STAXIS_AUTH_SECRET", "test-secret")
    session = make_session(tmp_path)
    register(session, "test@example.com", "correct horse battery")
