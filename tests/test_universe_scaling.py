...
def test_collection_pass_is_sliced_so_live_polling_is_not_starved(monkeypatch):
    monkeypatch.setenv("COLLECT_SECONDS", "600")
    monkeypatch.setenv("LIVE_POLL_SECONDS", "30")
    monkeypatch.setenv("STAXIS_COLLECT_SLICE_CHANNELS", "10")
    stop = Event()
    loads, chunks, live_calls = [], [], []
    ...
    def fake_collection(session, client, targets, **kwargs):
        chunks.append((len(targets), kwargs.get("intelligence", True)))
        if len(chunks) == 3:
            stop.set()
    ...
    monkeypatch.setattr(service, "poll_live_once", lambda *a, **k: live_calls.append(1) or service.LivePollResult(0, 0, 0, 0, 0))
    ...
    assert chunks == [(10, False), (10, False), (5, True)]  # scoring runs once, on the last slice
    assert len(loads) == 1  # one channel list per pass
    assert live_calls  # live polling ran between slices
