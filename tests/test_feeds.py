...
def test_long_running_stream_is_primary_and_the_rest_are_secondary():
    split = split_feeds([stream("main", 50_000, 24 * 45), stream("event", 12_000, 1), stream("panel", 3_000, 2)], now=NOW)
...
def test_all_feed_is_primary_plus_secondaries_even_when_secondary_is_larger():
    split = split_feeds([stream("main", 10_000, 24 * 40), stream("event", 90_000, 3)], now=NOW)
...
def test_no_long_running_stream_means_no_primary_and_all_is_not_understated():
    split = split_feeds([stream("a", 4_000, 2), stream("b", 6_000, 24 * 29)], now=NOW)
...
def test_earliest_started_eligible_stream_wins_and_ties_break_on_viewers():
    split = split_feeds([stream("newer", 90_000, 24 * 31), stream("older", 1_000, 24 * 60)], now=NOW)
    assert split.primary_video_id == "older"
    tie = split_feeds([stream("a", 1_000, 24 * 40), stream("b", 5_000, 24 * 40)], now=NOW)
...
def test_threshold_is_configurable(monkeypatch):
    ...
    assert split_feeds(streams, now=NOW).min_primary_hours == 720.0


def test_default_threshold_is_30_days():
    assert split_feeds([stream("m", 1, 24 * 30 - 1)], now=NOW).primary == 0
    assert split_feeds([stream("m", 1, 24 * 30)], now=NOW).primary == 1


def test_naive_datetimes_are_treated_as_utc_and_empty_input_is_zero():
    naive = LiveStream("m", 100, (NOW - timedelta(days=45)).replace(tzinfo=None))
