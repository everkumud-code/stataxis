from metrics.competition import (
    CompetitionPoint,
    build_competition,
...
def test_competition_ranks_by_value_and_preserves_missing_values():
    standings = build_competition(
        [CompetitionPoint("b", "Beta", 800), CompetitionPoint("a", "Alpha", 1000), CompetitionPoint("c", "Gamma", None)]
    )
...
def test_equal_values_use_competition_ranking():
    standings = build_competition(
        [CompetitionPoint("a", "Alpha", 100), CompetitionPoint("b", "Beta", 100), CompetitionPoint("c", "Gamma", 50)]
    )
...
def test_rank_share_and_gap_changes_are_calculated_from_previous_window():
    ...
    assert (alpha.gap_to_leader, alpha.previous_gap_to_leader, alpha.gap_change) == (200, 200, 0)
...
def test_head_to_head_reports_value_and_momentum_leaders():
    result = compare_head_to_head([
        CompetitionPoint("a", "Alpha", 1200, momentum=4),
        CompetitionPoint("b", "Beta", 900, momentum=7),
    ], "a", "b")
...
def test_head_to_head_preserves_ties_as_no_leader():
    result = compare_head_to_head([
        CompetitionPoint("a", "Alpha", 100, momentum=5),
        CompetitionPoint("b", "Beta", 100, momentum=5),
    ], "a", "b")
...
def test_head_to_head_does_not_invent_missing_values():
    result = compare_head_to_head([
        CompetitionPoint("a", "Alpha", None, momentum=5),
        CompetitionPoint("b", "Beta", 100, momentum=None),
    ], "a", "b")
    assert result.value_leader is None
